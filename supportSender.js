...
function createMainInterface() {
    ...
    if (
      ...
      html +=
        ` <td>
                        <input id="` +
        units[i] +
        `total" value="0" type="text"  class="totalTroops scriptInput"  disabled>
      ...
      html +=
        `
                    <td align="center" >
                        <input id="` +
        units[i] +
        `total" value="0" type="number"   class="scriptInput sendTroops" >
                        <font color="${textColor}" class="hideMobile">k</font>
                    </td>  `;
    }
  }
      ...
      html +=
        `
                    <td align="center" >
                        <input id="` +
        units[i] +
        `Reserve" value="0" type="number"   class="scriptInput reserveTroops" >
                        <font color="${textColor}" class="hideMobile">k</font>
                    </td>  `;
    }
  }
  ...
  if (
    localStorage.getItem(game_data.world + "support_sender_settings2") != null
  ) {
    //initialize checkbox
    let list_checkbox = JSON.parse(
      localStorage.getItem(game_data.world + "support_sender_settings2")
    )[0];
  ...
  //save settings
  $("#table_upload input[type=checkbox], #table_upload input").on(
    "click input change",
    () => {
      countTotalTroops();
      let list_checkbox = [];
      ...
      let data_localStorage = localStorage.getItem(
        game_data.world + "support_sender_settings2"
      );
      console.log(data);
...
function countTotalTroops() {
  ...
  Array.from($("#village_troup_list tbody tr")).forEach((row) => {
    ...
    units.forEach((troopName) => {
      let totalTroops = parseInt(
        $(row).find(`[data-unit='${troopName}']`).text()
      );
      let reserveTroops = parseFloat($(`#${troopName}Reserve`).val());

      reserveTroops =
        reserveTroops == undefined || Number.isNaN(reserveTroops) == true
          ? 0
          : reserveTroops * 1000;

      totalTroops =
        totalTroops > reserveTroops ? totalTroops - reserveTroops : 0;
      ...
      if (
        totalTroops > 0 &&
        dateStart.getTime() < date_current.getTime() &&
        date_current.getTime() < dateStop.getTime()
      ) {
  ...
  Array.from(mapVillages.keys()).forEach((key) => {
    ...
    if (
      obj["ram_speed"] != undefined ||
      obj["catapult_speed"] != undefined ||
      obj["sword_speed"] != undefined
    ) {
      ...
      obj["spear_speed"] != undefined ||
      obj["archer_speed"] != undefined
    ) {
      objTroopsTotal.spear += obj.spear;
      objTroopsTotal.heavy += obj.heavy;
      objTroopsTotal.spy += obj.spy;
  ...
  Object.keys(objTroopsTotal).forEach((key) => {
    if (
      ...
      if (units.includes(key)) {
        document.getElementById(key + "total").value = (
          objTroopsTotal[key] / 1000
        ).toFixed(2);
      }
...
function fillInputs() {
  ...
  let troopsTotal = Array.from(
    document.getElementsByClassName("totalTroops")
  ).map((e) => parseFloat(e.value) * 1000);
  let sendTotalObj = {};
  let sendTotal = Array.from(document.getElementsByClassName("sendTroops")).map(
    (e) => ({
      value: Number.isNaN(parseFloat(e.value) * 1000)
        ? 0
        : parseFloat(e.value) * 1000,
  ...
  let checkbox = document
    .getElementById("village_troup_list")
    .children[0].children[0].getElementsByTagName("input");
  ...
  Object.keys(factorTroopSent).forEach((troopName) => {
    ...
    listTotalRange.sort((o1, o2) => {
      return o1[troopName] > o2[troopName]
        ? 1
        : o1[troopName] < o2[troopName]
        ? -1
        : 0;
    });
    ...
    for (let i = 0; i < listTotalRange.length; i++) {
      ...
      if (troopValue < factorValue) {
        ...
        if (listTotalRange[i][troopName] + 1 > factorValue) {
          ...
          console.log(
            `troop name ${troopName}, value: ${listTotalRange[i][troopName]}`
          );
        }
      }
      let timeWindow = document.getElementById("checkbox_window").checked;
      ...
      if (
        listTotalRange[i]["speedTroop"] == troopName &&
        listTotalRange[i][troopName] == 0 &&
        timeWindow == true
      ) {
...
function addEvents() {
  ...
  $(".sendTroops").on("input", function (e) {
    ...
    document.getElementById("packets_send").value = (totalPop / 1000).toFixed(
      2
    );
  });
  ...
  $("#packets_send").on("input", function (e) {
    ...
    for (let i = 0; i < totalTroops.length; i++) {
      ...
      if (!id.includes("spy")) {
        sendTotal[i].value =
          parseInt(parseFloat(totalTroops[i].value) * ratio * 100) / 100.0;
      } else {
        sendTotal[i].value = 0;
      }
    }
...
function getSpeedConstant() {
  //Get speed constant (world speed * unit speed) for world
  if (localStorage.getItem(game_data.world + "speedWorld") !== null) {
    ...
    let unitSpeed = Number(
      htmlDoc.getElementsByTagName("unit_speed")[0].innerHTML
    );
