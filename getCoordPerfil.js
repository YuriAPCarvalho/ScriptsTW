(() => {
  try {
    ...
    localCoord[0] = parseInt(localCoord[0]);
    localCoord[1] = parseInt(localCoord[1]);
    localCoord = {
      x: localCoord[0],
      y: localCoord[1],
    };
    ...
    for (i = 0; i < coords.length; i++) {
      ...
      distOrderedCoords[i].label =
        distOrderedCoords[i].x + "|" + distOrderedCoords[i].y;
    }
    ...
    pointAscOrderedCoords.sort(
      (a, b) =>
        parseFloat(trimSeparators(a.points)) -
        parseFloat(trimSeparators(b.points))
    );

    var pointDescOrderedCoords = coords.slice();
    pointDescOrderedCoords.sort(
      (a, b) =>
        parseInt(trimSeparators(b.points)) - parseInt(trimSeparators(a.points))
    );
    ...
    coords.forEach((c, i) => {
      var $entry = $(
        '<td style="text-align: right" class="gc-col"><input type="checkbox" checked></td>'
      );
    ...
    var $listHeader = $(
      '<th style="width:80px;text-align:right;" class="gc-col">Include <input type="checkbox" id="gc-v-all" checked></th>'
    );
    $listHeader.change((e) => {
      console.log(e);
    ...
    $containerTable.append(
      '<tr><th colspan="2">Village Coordinates Export (Script)</th></tr>'
    );
    var dataStyle =
      "width: 80vw; max-width: 50em;  height: 5em; overflow:scroll;";
    ...
    function makeOptionsUI($table) {
      ...
      var separator =
        '<div style="display:inline-block;width:0.5em;"></div> | <div style="display:inline-block;width:0.5em;"></div>';

      $container.append(
        `<label for="gc-claims">Use [claim] instead of [coord]</label><input type="checkbox" id="gc-claims" ${
          options.useClaimsTag ? "checked" : ""
        }>`
      );
      $container.append(separator);
      $container.append(
        `<label for="gc-number">Numbering</label><input type="checkbox" id="gc-number" ${
          options.useNumbering ? "checked" : ""
        }>`
      );
      $container.append(separator);
      $container.append(
        `<label for="gc-points">Show points</label><input type="checkbox" id="gc-points" ${
          options.includePoints ? "checked" : ""
        }>`
      );
    ...
    function makeBbcodeList(coords, $table) {
      ...
      $row.append(
        `<td><textarea style="${dataStyle}">${bbCode}</textarea></td>`
      );
    ...
    function makeBigJsString(coords, $table) {
      ...
      var $row = $("<tr>");
      $row.append(
        `<td style="${labelStyle}">Script Input<br>(One Big Line)</td>`
      );
      $row.append(
        `<td><textarea style="${dataStyle}">"${output}"</textarea></td>`
      );
    ...
    function makeJsArray(coords, $table) {
      ...
      var $row = $("<tr>");
      $row.append(
        `<td style="${labelStyle}">Script Input<br>(Comma-Separated)</td>`
      );
      $row.append(
        `<td><textarea style="${dataStyle}">${JSON.stringify(output).replace(
          /'/g,
          ""
        )}</textarea></td>`
      );
