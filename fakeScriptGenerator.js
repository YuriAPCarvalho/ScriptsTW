...
var scriptConfig = {
  ...
  translations: {
    en_DK: {
      ...
      "Choose units and amounts to send": "Choose units and amounts to send",
      ...
      jQuery("#raGenerateFakeScriptBtn").on("click", function (e) {
        ...
        if (coordinatesFillMode === "manual") {
          ...
          if (playersInput.length === 0 && tribesInput.length === 0) {
            UI.ErrorMessage(
              twSDK.tt("You must select at least one player or one tribe!")
            );
            return;
          }
