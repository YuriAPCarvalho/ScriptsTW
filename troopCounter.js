...
*/
javascript: if (!licznik_wojska) var licznik_wojska = {};
var langScript = [];
if (game_data.locale == "en_DK") {
  ...
  licznik_wojska.nazwyJednostek =
    "Spear_fighter,Swordsman,Axeman,Archer,Scout,Light_cavalry,Mounted_archer,Heavy_cavalry,Ram,Catapult,Paladin,Nobleman".split(
      ","
    );
}
if (game_data.locale == "pt_BR") {
  ...
  licznik_wojska.nazwyJednostek =
    "Lanceiro,Espadachim,Bárbaro,Arqueiro,Explorador,Cavalaria_Leve,Arqueiro_a_cavalo,Cavalaria_Pesada,Aríete,Catapulta,Paladino,Nobres".split(
      ","
    );
}
var tabela;
...
licznik_wojska.link =
  ...
  "&type=complete&mode=units&group=0&page=-1&screen=overview_villages";
...
licznik_wojska.obrazki =
  "spear,sword,axe,archer,spy,light,marcher,heavy,ram,catapult,knight,snob".split(
    ","
  );
...
var okienko =
  ...
  langScript[1] +
  "<select id='listaGrup' onchange=\"licznik_wojska.link = this.value; pobierzDane();\"><option value='" +
  licznik_wojska.link +
  "'>" +
  langScript[2] +
  "</select>";
okienko +=
  "<tr><td><table width='100%'><tr><th colspan='4'>" +
  langScript[3] +
  "<select onchange=\"zmiana(this.value);\"><option value='0'>" +
  langScript[4] +
  "<option value='0p2p3'>" +
  langScript[5] +
  "<option value='1'>" +
  ...
  "<option value='3'>" +
  langScript[9] +
  "</select><tbody id='dostepne_wojska'></table><tr><th><b id='ilosc_wiosek'></b><a href='#' style='float: right;' onclick=\"eksportuj();\">" +
  langScript[10] +
  "</a></table>";
      ...
      if (!licznik_wojska.pobraneGrupy) {
        for (i = 0; i < grupy.length; i++) {
          ...
          $("#listaGrup").append(
            $("<option>", {
              value:
                grupy[i].getAttribute(mobile ? "value" : "href") + "&page=-1",
        ...
        if (!tabela.rows[0].innerHTML.match("archer")) {
          licznik_wojska.obrazki.splice(
            licznik_wojska.obrazki.indexOf("archer"),
            1
          );
          licznik_wojska.obrazki.splice(
            licznik_wojska.obrazki.indexOf("marcher"),
            1
          );
        }
        if (!tabela.rows[0].innerHTML.match("knight"))
          licznik_wojska.obrazki.splice(
            licznik_wojska.obrazki.indexOf("knight"),
            1
          );
      }
      sumuj();
...
function zmiana(tekst) {
  ...
  var nowaSuma = [];
  for (j = 0; j < licznik_wojska.obrazki.length; j++) nowaSuma[j] = 0;
  for (i = 0; i < ktory.length; i++)
    if (i == 0 || coZrobic[i - 1] == "p")
      nowaSuma = dodaj(nowaSuma, sumaWojsk[ktory[i]]);
    else nowaSuma = odejmij(nowaSuma, sumaWojsk[ktory[i]]);
  wypisz(nowaSuma);
}
...
function rysujSpacje(ile) {
  ...
  var wynik = "";
  for (j = 0; j < 10 - tekst.length; j++) wynik += "\u2007";
  return wynik;
}

function wypisz(sumaWojskDoWypisania) {
  elem = "<tr>";
  licznik_wojska.eksport =
    "<textarea rows='7' cols='25' onclick=\"this.select();\">";
  for (i = 0; i < licznik_wojska.obrazki.length; i++) {
    licznik_wojska.eksport +=
      "[unit]" +
      licznik_wojska.obrazki[i] +
      "[/unit]" +
      sumaWojskDoWypisania[i] +
      (i % 2 == 0 ? rysujSpacje(sumaWojskDoWypisania[i]) : "\n");
    elem +=
      (i % 2 == 0 ? "<tr>" : "") +
      "<th width='20'><a href='" +
      langScript[15] +
      licznik_wojska.nazwyJednostek[i] +
      "' target='_blank'><img src='" +
      image_base +
      "unit/unit_" +
      licznik_wojska.obrazki[i] +
      ".png'></a><td bgcolor='#fff5da'>" +
      sumaWojskDoWypisania[i];
  }
  ...
  $("#ilosc_wiosek").html(
    langScript[16] + (tabela.rows.length - 1) / 5 + langScript[17]
  );
}
