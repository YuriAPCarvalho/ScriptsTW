if (document.URL.match("mode=incomings&type=unignored&subtype=attacks")) {
    ...
    $.ajax({
      ...
      success: function () {
        ...
        if (commandName.includes("Espada")) {
          ...
          commandName.includes("Machado") ||
          commandName.includes("Lanceiro")
        ) {
          var remainingFields = seconds / unitSpeed[1];
        } else if (
          commandName.includes("Batedor") ||
          commandName.includes("Btd")
        ) {
          var remainingFields = seconds / unitSpeed[2];
        } else if (
          commandName.includes("CavL") ||
          commandName.includes("Leve")
        ) {
          var remainingFields = seconds / unitSpeed[3];
        } else if (
          commandName.includes("CavP") ||
          commandName.includes("Pesada")
        ) {
          var remainingFields = seconds / unitSpeed[4];
        } else if (
          commandName.includes("ArÃ­ete") ||
          commandName.includes("Cata") ||
          commandName.includes("Catapulta")
        ) {
        ...
        var remaining =
          Math.sqrt(
            Math.pow(M[0] - source[0], 2) + Math.pow(M[1] - source[1], 2)
          ) - currentDistance;
  ...
  self.location = game_data.link_base_pure.replace(
    /screen\=\w*/i,
    "screen=overview_villages&mode=incomings&type=unignored&subtype=attacks"
  );
}
