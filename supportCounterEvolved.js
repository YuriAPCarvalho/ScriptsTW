...
var scriptConfig = {
  ...
  translations: {
    en_DK: {
      ...
      "Support in other players villages": "Support in other players villages",
      ...
      "Toggle call back by unit amounts": "Toggle call back by unit amounts",
...
$.getScript(
  ...
  async function () {
    ...
    function initSingleVillageScreen() {
      ...
      setTimeout(() => {
        const amount =
          localStorage.getItem(`${scriptConfig.scriptData.prefix}_amount`) ?? 0;
        if (amount) {
    ...
    function handleFillUnits() {
      jQuery("#raSelectAmountOfUnitsBtn").on("click", function (e) {
        ...
        const isUnitAmountCallBackActive = jQuery("#raToggleCallBackMethod").is(
          ":checked"
        );
        ...
        if (isUnitAmountCallBackActive) {
          ...
          jQuery(".ra-unit").each(function () {
            ...
            const unitType = jQuery(this).attr("name").split("unit_")[1];
            if (unitAmount > 0) {
              unitAmounts = {
                ...unitAmounts,
                [unitType]: unitAmount,
          ...
          if (canCallBack) {
            ...
            chosenUnits.forEach((unit) => {
              const unitFieldAmount = jQuery(`.unit-item-${unit}`).not(
                ".hidden"
              ).length;
              unitFieldAmounts = {
                ...unitFieldAmounts,
            ...
            jQuery("#withdraw_selected_units_village_info tbody tr").each(
              function (_, tableRow) {
                const isOwnSupport = jQuery(tableRow).find("td:eq(0) a").length;
                if (isOwnSupport === 1) {
                  jQuery(tableRow)
                      ...
                      if (chosenUnits.includes(unit)) {
                        jQuery(this)
                          ...
                          .val(
                            Math.ceil(
                              unitAmounts[unit] /
                                (unitFieldAmounts[unit] - outsideDefRows)
                            )
                          );
                      } else {
                        jQuery(this).find(".call-unit-box").val(0);
                      }
                    });
                }
              }
                  ...
                  .each(function () {
                    ...
                    if (chosenUnits.includes(unit)) {
                      const unitAmount = parseInt(
                        jQuery(this).find(".call-unit-box").val()
                      );
                      jQuery(this)
                        .find(".call-unit-box")
                        .val(Math.floor(unitAmount * (amount / 100)));
                    } else {
                      jQuery(this).find(".call-unit-box").val(0);
                    }
                  });
    ...
    function handleUnitChoserToggle() {
      jQuery(".ra-unit-selector").on("change", function (event) {
        if (event.target.checked) {
          jQuery(`.ra-unit-${event.target.value}`).prop("disabled", false);
        } else {
    ...
    function buildUnitAmountSelector() {
      const storedChosenUnits = JSON.parse(
        localStorage.getItem(`${scriptConfig.scriptData.prefix}_chosen_units`)
      ) ?? ["spear", "sword"];
    ...
    async function fetchSupportsData() {
      ...
      try {
        ...
        UI.ErrorMessage(twSDK.tt("There was an error fetching supports data!"));
    ...
    function calculateSupport(element) {
      ...
      tableRows.each((i, row) => {
        ...
        for (let i = 1; i < rowData.length - 1; i++) {
          ...
          const unitPopAmount =
            unitName !== "heavy" ? twSDK.unitsFarmSpace[unitName] : hcPopAmount;
